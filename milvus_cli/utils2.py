...
def getMilvusTimestamp(isSimilar: bool = True) -> bool:
    ts = time()
    if isSimilar:
        return int(ts) << 18
...
class PyOrm:
    ...
    def __init__(self, settings: OrmSettings = OrmSettings()):
    ...
    def connect(self, alias: t.Optional[str] = None, host: t.Optional[str] = None, port: t.Optional[int] = None, disconnect: bool = False):
        self.alias = alias
        self.host = host
        self.port = port
    ...
    def showConnection(self, alias: str = "default", showAll: bool = False) -> str:
        from pymilvus import connections
    ...
    def _list_collection_names(self, timeout: t.Optional[float] = None) -> t.List[str]:
        from pymilvus import list_collections

        return list(list_collections(timeout, self.alias))

    def _list_partition_names(self, collectionName: str) -> t.List[str]:
        target = self.getTargetCollection(collectionName)
        result = target.partitions
        return [i.name for i in result]

    def _list_field_names(self, collectionName: str, showVectorOnly: bool = False):
        target = self.getTargetCollection(collectionName)
    ...
    def _list_index(self, collectionName: str) -> t.Dict[str, t.Any]:
        target = self.getTargetCollection(collectionName)
        try:
            result = target.index()
        except Exception as e:
